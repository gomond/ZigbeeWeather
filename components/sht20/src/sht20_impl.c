...
static esp_err_t sht20_read_raw(i2c_port_t i2c_num, uint8_t addr, uint8_t cmd, uint16_t *out_raw)
{
    ...
    // Verify CRC-8 (Sensirion polynomial 0x31) for first two bytes
    uint8_t expected_crc = data[2];
    // CRC-8 calculation (MSB-first, poly 0x31)
    uint8_t crc = 0;
    for (int i = 0; i < 2; ++i) {
        crc ^= data[i];
        for (int b = 0; b < 8; ++b) {
    ...
    if (crc != expected_crc) {
        ESP_LOGE(TAG, "CRC mismatch: calc=0x%02x expected=0x%02x", crc, expected_crc);
